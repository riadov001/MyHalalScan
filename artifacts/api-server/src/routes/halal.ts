...
router.get("/halal/analyze/:barcode", async (req, res) => {
  ...
  // Step 4: Product not found in any source — try UPCitemdb for a name, then return unknown
  if (!product) {
    const productName = isNumericBarcode
      ? await fetchProductNameFromUPCItemDB(barcode)
      : null;
    req.log.info({ barcode, productName }, "Product not found in any source");
    res.json({
      result: "unknown",
      productName: productName || "Produit non référencé",
      reason: productName
        ? `Produit trouvé (${productName}) mais ingrédients non disponibles`
        : "Ce produit n'est pas référencé dans les bases de données disponibles",
      foundInDatabase: !!productName,
